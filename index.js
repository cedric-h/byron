...
(function main() {
  ...
  const buf = {
    idx_cpu_i: 0,
    idx_cpu: new Uint16Array(1 << 12),
    idx_gpu: gl.createBuffer(),
    vrt_cpu_i: 0,
    vrt_cpu: new Float32Array(1 << 12),
  ...
  function buf_cube(buf, { pos, scale, color, shadow }) {
    ...
    let cube_i = 0;
    for (let i = 0; i < CUBE.vrt.length/3; i++) {
      let x = pos[0] + CUBE.vrt[cube_i++]*scale;
      let y = pos[1] + CUBE.vrt[cube_i++]*scale;
      let z = pos[2] + CUBE.vrt[cube_i++]*scale;
      if (shadow) {
        if (y < 0) y = 0.001;
    ...
    for (const i of CUBE.idx)
      buf.idx_cpu[buf.idx_cpu_i++] = cube_start_vrt_i+i;
  }

  buf_cube(buf, { pos: [0, 1, 0], scale: -1.00, color: 0 });
  buf_cube(buf, { pos: [0, 1, 0], scale:  0.95, color: 1 });
  buf_cube(buf, { pos: [0, 1, 0], scale:  1.00, color: 0.8, shadow: true });
